...
/*
    ...
    status: {type: String, required: true, enum: ['還沒看過', '看過:借閱','看過:收藏中'], default: '尚未看過'},
    review_star: {type:Number, min: 1, max: 5},
    review_summary: {type: String },
    review_date:{ type: Date, default: Date.now }
    */

function bookCreate(no, title, author, status, review_star, review_summary, review_date , cb) {
  bookdetail = { 
    no: no,
    title: title,
    author: author, 
...
// book schema : ( NO,title,author,status,review_star,review_summary,review_date )
function createBooks(cb) {
    async.parallel([
        function(callback) {
          bookCreate(1, '打敗噴火龍','原裕', '看過:收藏中',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(2, '恐怖的鬼屋','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(3, '魔法師的弟子','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(4, '海盜尋寶記  ','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(5, '媽媽我愛你','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(6, '邪惡幽靈船','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(7, '勇闖巧克力城','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(8, '恐怖遊樂園','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },    
        function(callback) {
          bookCreate(9, '拯救小恐龍','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(10, '大怪獸入侵','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(11, '恐怖的禮物','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(12, '恐怖足球隊','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(13, '佐羅力被捕了','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(14, '神秘的飛機','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(15, '妖怪大作戰','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(16, '忍者大作戰','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(17, '佐羅力要結婚？！','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(18, '大戰佐羅力城','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(19, '恐怖的賽車','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },
        function(callback) {
          bookCreate(20, '恐怖大跳躍','原裕', '還沒看過',4, '-','2020/04/01', callback );
        },                             
        ],
        // optional callback
        cb);
